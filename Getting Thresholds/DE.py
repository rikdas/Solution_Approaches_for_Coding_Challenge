...
def DE(objf, lb, ub, dim, PopSize, iters):
    ...
    while t < iters:
        ...
        try:
            if(round(convergence_curve[t],4)<round(convergence_curve[t-1],4)): s.conv_gen=t+1
        except:
            pass
        ...
        # for detailed report
        if t % 1 == 0:
            print("Iteration", t + 1, "Best Fitness :", round(s.best, 3))
            print('[ ', end='')
            for myItter in s.leader_solution:
                print(int(myItter), end=', ')
            print(']')
        '''
